...
"""Parser for each news and annoucements articules.
  ...
  News:
    ...
    'date' string: Data publikacji podana przez MS
...
for i in range(1, pages_n + 1):
    url = f'https://www.gov.pl/web/sprawiedliwosc/aktualnosci-wydarzenia?page={str(i)}&size=10'
...
for i in range(1, pages_a + 1):
    url = f'https://www.gov.pl/web/sprawiedliwosc/aktualnosci-ogloszenia-i-informacje?page={str(i)}&size=10'
    page = requests.get(url)
    if page.status_code != 200:
        break
    soup = BeautifulSoup(page.content, 'html.parser')
